...
#else
#define sprd_wifi_preload_driver() do { } while (0)
#define sprd_wifi_after_unload_driver() do { } while (0)
#endif
...
/* < Bug#575455 modify insmod implementation for new sepolicy */
#if 0
...
int wifi_change_driver_state(const char *state)
{
    ...
    if (fd < 0) {
        LOGE("Failed to open driver state control param (%s)", strerror(errno));
        return -1;
    }
    len = strlen(state) + 1;
    if (TEMP_FAILURE_RETRY(write(fd, state, len)) != len) {
        LOGE("Failed to write driver state control param (%s)", strerror(errno));
        ret = -1;
    }
    close(fd);
    return ret;
...
//<-- Bug#479595 Add for WCND RESET Feature END
    ...
    LOGE("wifi_start_supplicant1");
    /* Check whether already running */
    if (property_get(supplicant_prop_name, supp_status, NULL)
            && strcmp(supp_status, "running") == 0) {
        return 0;
    }
    LOGE("wifi_start_supplicant2");
    /* Before starting the daemon, make sure its config file exists */
    if (ensure_config_file_exists(SUPP_CONFIG_FILE) < 0) {
        LOGE("Wi-Fi will not be enabled");
        return -1;
    }
    LOGE("wifi_start_supplicant3");
	...
	LOGE("wifi_start_supplicant5");
    property_get("wifi.interface", primary_iface, WIFI_TEST_INTERFACE);

    property_set("ctl.start", supplicant_name);
	LOGE("supplicant_name = %s",supplicant_name);	
    sched_yield();
	LOGE("wifi_start_supplicant6");
    while (count-- > 0) {
	
        if (pi == NULL) {
            pi = __system_property_find(supplicant_prop_name);
        }
	LOGE("wifi_start_supplicant6 pi = %s",pi);		
        if (pi != NULL) {
            /*
...
restart:
    *reply_len = buffer_len;
    ...
    rfds[1].events |= POLLIN;
    do {
        res = TEMP_FAILURE_RETRY(poll(rfds, 2, timeout));
        if (res < 0) {
            LOGE("Error poll = %d", res);
            return res;
        } else if (res == 0) {
             ...
             * or not ..
             */
            LOGE("wait for event %s timed out", event);
            res = wifi_supplicant_connection_active();
            if (res < 0) {
                LOGE("supplicant connection lost");
                return -2;
            }
            return -3;
        }
    ...
    if (rfds[0].revents & POLLIN) {
        wpa_ctrl_recv(monitor_conn, reply, reply_len);
        ALOGD("recevied event: %s", reply);
        if (strstr(reply, event)) {
            ALOGD("got target event: %s", event);
            return 0;
        }
        goto restart;
    }
...
//NOTE: Bug#479595 Add for WCND RESET Feature BEG -->
    ...
    if (strstr(buf, WPA_EVENT_TERMINATING)) {
        ...
        if (property_get("persist.vendor.sys.wcn.status", cp2_status, "ok")) {
