...
#define TEXT_SENSOR_FACTORY_CALL_LP "get sensor data"
...
int getSensorData_lp(sensors_event_t *pEvent)
{
    ...
    ui_set_color(CL_GREEN);
    ui_show_text(open_row, 0, TEXT_LSENSOR_OPENING);
    gr_flip();
    ...
    if(light_pass == 1){
        ...
        test_result = RL_PASS;
        enable_sensor(DISABLE_SENSOR , SENSOR_TYPE_LIGHT);
    }else
            test_result = RL_FAIL;
    return 0;
}
...
static void *test_Ppsensor_thread (void *)
{
    int n = 0;
    char buff[256] = {0};
    int ret = -1;
    ...
    do {
        now_time=time(NULL);
        LOGD("test_lpsensor_thread mmitest here while notify_sensor_data=%d",notify_sensor_data);
        LOGD("test_lpsensor_thread mmitest here while now_time=%d,start_time=%d",now_time,start_time);
        if (notify_sensor_data){
            ret = 1;
        }
        if((now_time-start_time) >= SENSOR_TEST_TIMEOUT) {
...
int test_lsensor_start_extern(void)
{
    ...
    pthread_create(&thread, NULL, test_lpsensor_thread, NULL);
    usleep(10 * 1000);
    //ret = ui_handle_button(NULL,NULL,NULL);//, TEXT_GOBACK
    //Register callback
    LOGD("FT: add callback: %x", getSensorData_lp);
    chnl_fw_ptrFunc_add(TEXT_SENSOR_FACTORY_CALL_LP, (void **)(&getSensorData_lp));

    enable_sensor(ENABLE_SENSOR , SENSOR_TYPE_LIGHT);
    ...
    gr_flip();
    sleep(1);
    chnl_fw_ptrFunc_remove(TEXT_SENSOR_FACTORY_CALL_LP);
    LOGD("exit");
    save_result(CASE_TEST_LSENSOR, test_result);
    return ret;
}
