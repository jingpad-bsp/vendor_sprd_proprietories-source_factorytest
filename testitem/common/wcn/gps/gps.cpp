...
static void   gnss_sv_status_callback_test(GnssSvStatus* sv_status);
...
int gpsOpen( void )
{
	...
	if (remove("data/gps_outdirfile.txt") <= 0){
	    LOGD("remove gps_outdirfile.txt fail!");
	}
	if (system("rm /data/gps/log/*") <= 0){
	    LOGD("remove gps log file fail!");
	}
...
static void gps_show_result(unsigned int result)
{
	...
	if(result == 1){
		...
		snprintf(buffer, sizeof(buffer), GPS_TEST_PASS);
	}else if(result == 0){
		ui_clear_rows(row, 2);
		memset(buffer, 0, sizeof(buffer));
		snprintf(buffer, sizeof(buffer), GPS_TEST_FAILED);

	}else if(result == 2){
		ui_clear_rows(row, 2);
		memset(buffer, 0, sizeof(buffer));
		snprintf(buffer, sizeof(buffer), "%s, %ds", GPS_TESTING, sTimeout);
	}else{
	...
	ui_set_color(CL_WHITE);
	snprintf(buffer, sizeof(buffer), "GPS NUM: %d",sSVNum);
	row = ui_show_text(row, 0, buffer);
	gr_flip();
	for(i=0;i<sSVNum;i++){
        memset(buffer, 0, sizeof(buffer));
        snprintf(buffer, sizeof(buffer), "GPS SNR: %d",sSvSnr[i]);
        row = ui_show_text(row, 0, buffer);
	}
	gr_flip();
}
...
int gps_result=0;
static void * processThread_show(void *)
{
	int snr_count=0;
	int snr_count1=0;
	int i;
...
static void gnss_sv_status_callback_test(GnssSvStatus* sv_status)
{
	int i=0;
	FUN_ENTER;
	LOGD("mmitest GnssSvStatus");
	if( NULL != sv_status )
	{
		last_sSVNum = sSVNum;
		sSVNum = sv_status->num_svs;
		sPreTest = sSVNum;
		LOGD("gps callback: sPreTest = %d", sPreTest);

		//LOGD("mmitest GpsSvStatus: num_svs  = %d\n", sv_status->num_svs);
		for(; i < sv_status->num_svs; ++i ) {
			sSvSnr[i]=sv_status->gnss_sv_list[i].c_n0_dbhz;
			SPRD_DBG("GnssSvStatus lst[%d]: prn       = %d",   i, sv_status->gnss_sv_list[i].svid);
			SPRD_DBG("GnssSvStatus lst[%d]: snr       = %02f", i, sv_status->gnss_sv_list[i].c_n0_dbhz);
			SPRD_DBG("GnssSvStatus lst[%d]: elevation = %02f", i, sv_status->gnss_sv_list[i].elevation);
			SPRD_DBG("GnssSvStatus lst[%d]: azimuth   = %02f", i, sv_status->gnss_sv_list[i].azimuth);
		}
	}
	FUN_EXIT;
}
...
*/
  pthread_t pid;
  LOGD("%s called\n",__FUNCTION__);
  pthread_create(&pid, NULL, (void *(* _Nonnull)(void *))start, arg);

	FUN_EXIT;
	return pid;
}

int test_gps_pretest_common(void)
{
	int ret;
	if(sPreTest < 1){
		LOGD("test_gps_pretest_common do pretest!");
		gpsOpen();
		sleep(2);
		gpsClose();
	}
	...
	LOGD("pretest:sPreTest = %d", sPreTest);
	save_result(CASE_TEST_GPS,ret);
	return ret;
}

int test_gps_start_common(void)
{
	int ret;
	pthread_t t1;
	int row = 2;
	...
	gOpenTime=time(NULL);
	thread_run = 1;
	pthread_create(&t1, NULL, processThread_show, NULL);
	usleep(10*1000);
