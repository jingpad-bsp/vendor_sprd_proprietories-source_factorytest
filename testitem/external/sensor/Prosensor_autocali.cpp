...
static int SENSOR_TYPE = SENSOR_TYPE_PROXIMITY;
static int SENSOR_TEST_RESULT = CASE_CALI_PROSOR;
static char* TEST_TITLE = "Proximity Sensor";
static char* TEST_MENU = MENU_CALI_AUTOPROSOR;
static char* TEST_TEXT_OPER = PRO_CALI_OPER1;
...
static void *test_auto_psensor_cali_thread (void *)
{
    test_result = do_cali_work();
    thread_run = 0;
    LOGD("test_psensor_cali_thread exit! test_result=%d",test_result);
    return NULL;
}
...
int cali_auto_prosensor_start_extern(void)
 {
	 ...
	 pthread_t thread,thread_timeout;
       ...
       //work start
       //ret = do_cali_work();
	pthread_create(&thread, NULL, test_auto_psensor_cali_thread, NULL);
	thread_run = 1;
	...
	LOGD("cali_prosensor_start_extern test_result=%d",test_result);
	ret = test_result;
