...
static display_content info[100];

#define AT_MEID "AT+SPMEID?"
#define AT_MEID_RSP_Prefix "+SPMEID:"

#define MAX_TEXT_LEN 1024

int showModemVersion(char * modem_ver_info,int cur_row)
{
    char *delims={ "\n" };
    int count = 0;
    int ret = -1;
    ...
    char *p;
    char *buffer;
    buffer = strdup(modem_ver_info);
    LOGD("showModemVersion: buffer=%s",buffer);
    p = strtok(buffer,delims);
    int delimsed = 0;
    do{
        LOGD("FT: showModemVersion: word=%s",p);
...
int test_version_show_common(void)
{
    char androidver[MAX_TEXT_LEN];
    char tmpbuf[PROPERTY_VALUE_MAX];
    char sprdver[MAX_TEXT_LEN];
    char kernelver[MAX_TEXT_LEN];
    char* modemver;
    int fd, rd_size = 0;
    int cur_row = 2;
    int ret = 0;
    ...
    else
    {
        ...
        rd_size = read(fd, kernelver, sizeof(kernelver));
        if(rd_size < 0)
            LOGE("read error");
        else
    ...
    //showModemVersion by \n
    showModemVersion(modemver, cur_row);

    gr_flip();

    while(ui_handle_button(NULL, NULL, TEXT_GOBACK) != RL_FAIL);

    return ret;
}

#define PROP_MISCDATA_PARTIONPATH "ro.vendor.product.partitionpath"
...
void  getMEID(int phoneId, char *string)
{
    int ret = -1;
    char buff[64];
    memset(buff, 0, sizeof(buff));
    ret = sendATCmd(phoneId ,AT_MEID,buff,sizeof(buff),0);
    if(ret < 0){
        LOGD("getMEID MEID ret: %d IN %d", ret,  __LINE__);
        return;
    }
    char* temp = substring(buff, strlen(AT_MEID_RSP_Prefix) , strlen(buff) - strlen(AT_MEID_RSP_Prefix));
    memcpy(string, temp, strlen(temp));
    LOGD("getMEID MEID: %s IN %d", string,  __LINE__);
    free(temp);
    return ;
}

int test_phone_info_show_common(void)
{
    ...
    if ( 32 == test_modem_getlteconf() ){
        getMEID(0, meid1);
        LOGD("mmitest meid1=%s",meid1);
        getMEID(1, meid2);
        LOGD("mmitest meid2=%s",meid2);
        info[n].color = CL_WHITE;
        info[n++].content = TEXT_MEID;

        info[n].color = CL_GREEN;
        info[n++].content = meid1;
        info[n].color = CL_GREEN;
        info[n++].content = meid2;
    }
