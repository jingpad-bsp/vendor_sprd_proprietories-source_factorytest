...
pthread_t t_Produce_data = -1;
pthread_t t_Msensor_show = -1;
...
void sigalrm_func(int sig){
    if (success == 0 && hasData == 0){
        ui_push_result(RL_FAIL);

        pthread_kill(t_Produce_data,SIGUSR1);
        pthread_kill(t_Msensor_show,SIGUSR1);
    }
...
static void *msensor_thread(void *)
{
	int i;
	int data_num = -1;
	int type = 0;
	...
	type = getSensorType(sensor_id);
	LOGD("activate sensor: %s success! type = %d.", getSensorName(sensor_id), type);
	...
	do
	{
		if(first_time)		//at the first time, wait 1 sec and discard time data
		{
			...
			data_num = sensor_poll(buffer, numEvents);
		...
		for (i = 0; i < data_num; i++)
		{
			...
			if (type == data.type)
			{
				...
				if(fabsf(first_data.data[0]-data.data[0]) > 20)		// X axis fit the result
					xyz = xyz | 0x01;

				if(fabsf(first_data.data[1]-data.data[1]) > 20)		// Y axis fit the result
					xyz = xyz | 0x02;

				if(fabsf(first_data.data[2]-data.data[2]) > 20)		// Z axis fit the result
					xyz = xyz | 0x04;
			}
		}
...
int test_msensor_start_common(void)
{
	int ret = 0;
	...
	else
	{
		...
		setAlarm(5);
		pthread_create(&t_Produce_data, NULL, msensor_thread, NULL);
		pthread_create(&t_Msensor_show, NULL, Msensor_show, NULL);
