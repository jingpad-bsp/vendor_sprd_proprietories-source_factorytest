...
static void *gsensorcali_thread(void *)
{
	int i,n;
	int type_num = 0;
...
int cali_gsensor_start_common(void)
{
	pthread_t thread = -1;
	const char *ptr = getSensroTypeName(SENSOR_GYROSCOPE);
	int ret;
	int err;
	int cali_fd;
...
//	cur_row = ui_show_text(cur_row, 0, BOARD_HAVE_ACC);
	...
	thread_run=1;
	pthread_create(&thread, NULL, gsensorcali_thread, NULL);
	sleep(4);
