...
static void *prosensorcali_auto_thread(void *)
{
    int i, n;
    int type_num = 0;
    ...
    do
    {
        ...
        {
            data = buffer[i];
            LOGD("version=%d, expected=%d", data.version, sizeof(sensors_event_t));
...
int cali_auto_prosensor_start_common(void)
{
    pthread_t thread;
    ...
    thread_run = 1;
    pthread_create(&thread, NULL, prosensorcali_auto_thread, NULL);
    sleep(4);
