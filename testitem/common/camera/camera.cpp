...
typedef enum {
  CAMERA_BACK_MAIN = 0,
  CAMERA_FRONT_MAIN = 1,
  CAMERA_BACK_SECOND = 2,
  CAMERA_BACK_THIRD = 3,
  CAMERA_FRONT_SECOND = 4
} CAMERA_TYPE;


void showTitle(int camera_type){
    switch (camera_type) {
          case CAMERA_BACK_MAIN:
           ui_fill_locked();
            ui_show_title(MENU_TEST_BCAMERA);
           break;
          case CAMERA_FRONT_MAIN:
           ui_fill_locked();
            ui_show_title(MENU_TEST_FCAMERA);
           break;
          case CAMERA_BACK_SECOND:
           ui_fill_locked();
            ui_show_title(MENU_TEST_ACAMERA);
           break;
          case CAMERA_BACK_THIRD:
           ui_fill_locked();
            ui_show_title(MENU_TEST_THIRD_CAMERA);
           break;
          case CAMERA_FRONT_SECOND:
           ui_fill_locked();
...
void save_result_common(int camera_type, char key_result){
    switch (camera_type) {
          case CAMERA_BACK_MAIN:
         save_result(CASE_TEST_BCAMERA, key_result);
            break;
          case CAMERA_FRONT_MAIN:
         save_result(CASE_TEST_FCAMERA, key_result);
            break;
          case CAMERA_BACK_SECOND:
         save_result(CASE_TEST_ACAMERA, key_result);
            break;
          case CAMERA_BACK_THIRD:
         save_result(CASE_TEST_FACAMERA, key_result);
            break;
          case CAMERA_FRONT_SECOND:
         save_result(CASE_TEST_FACAMERA, key_result);
           break;
...
int test_camera_start_comm(void)
{
    int rtn = RL_FAIL;
...
//back camera test
int test_bcamera_start_common(void)
{
    int  rtn = RL_FAIL;
...
//front camera test
int test_fcamera_start_common(void)
{
    int  rtn = RL_FAIL;
...
int test_acamera_start_common(void)
{
    int  rtn = RL_FAIL;
...
int test_facamera_start_common(void)
{
    int  rtn = RL_FAIL;
...
//common camera
int test_camera_start_common(int cameraID)
{
    int  rtn = RL_FAIL;

    camera_id = cameraID;
    LOGD("test_camera_start_common, cameraID= %d",camera_id);
    if(camera_id == 3){
        showTitle(CAMERA_BACK_THIRD);
    }else{
        showTitle(CAMERA_BACK_MAIN);
    }
    if(camera_test_init())
    {
        LOGE("camera test fail");
    ...
    if(camera_id == 3){
        save_result(CASE_TEST_THIRD_CAMERA, rtn);
    }else{
        save_result(CASE_TEST_BCAMERA, rtn);
    }
