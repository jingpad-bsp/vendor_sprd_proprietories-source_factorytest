...
static void* tp_thread_show(void *)
{
    while(thread_run){
        gr_flip();
        usleep(1000 * 30);
...
static void* tp_thread(void *)
{
    ...
    support_ts = get_sensor_ts_name();
    if(strlen(support_ts))
    {
        fd = get_sensor_name(support_ts);
        if(fd < 0)
        {
            LOGD("mmitest open %s tp fail", support_ts);
            goto exit;
        }
    }
    else
    {
        LOGD("mmitest get tp event %s failed", support_ts);
        goto exit;
    }
    /* @} */
...
int test_tp_start_common(void)
{
    ...
    pthread_create(&tp_t_show, NULL,tp_thread_show, NULL);
    pthread_create(&tp_t, NULL, tp_thread, NULL);
