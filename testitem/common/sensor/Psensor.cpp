...
static void psensor_show()
{
	...
	snprintf(buf, sizeof(buf), "%s %5.1f hPa", TEXT_PrS_Value, pressure_value);
...
static void *psensor_thread(void *)
{
	int cnt=0;
	int i,n;
	int type_num;
...
//	sensors_event_t data_last={0};
	...
	LOGD("activate sensor success!!!: sensor name = %s, type_num = %d;", getSensorName(sensor_id), type_num);
...
int test_psensor_start_common(void)
{
	int ret = 0;
	const char *ptr = getSensroTypeName(SENSOR_PRESSURE);
	pthread_t thread;
	cur_row = 2;
