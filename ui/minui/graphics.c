...
extern void freetype_free(struct FontGraph *);
...
char* gr_text(int x, int y, const char *s)
{
    ...
    while(*s)
    {
        ...
        if(bytes <= 0)
        {
            LOGE("utf8towc error, bytes=%d", bytes);
            break;
        }
        else
            s += bytes;
        ...
        if(graph.width >0 && graph.height > 0)
        {
            ...
            if(x >= gr_fb_width())      //can not display in 1 line, return the remain text
            {
                LOGD("more than 1 lines, return remain text");
                freetype_free(&graph);
                return (s-bytes);
            }
            else
...
void gr_texticon(int x, int y, GRSurface* icon) {
    ...
    text_blend(src_p, icon->row_bytes, dst_p, gr_draw->row_bytes, icon->width, icon->height);
}
...
int gr_initRotate()
{
    if (s_rotate_type == ROTATE_UNKNOWN)
    {
        ...
        property_get("ro.vendor.sf.hwrotation", rotate_str, "0");
        printf("in %s: ro.vendor.sf.hwrotation=%s\n",__func__,rotate_str);
