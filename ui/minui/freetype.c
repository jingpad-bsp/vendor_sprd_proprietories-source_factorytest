...
//#define FREETYPE_LIB "/vendor/lib/libft2.so"
#define FREETYPE_LIB "/vendor/lib/lib_factorytest_ft2.so"
#define FONTS_PATH "/vendor/etc/NotoSansCJK-Regular-sprd.ttc"
...
pthread_mutex_t freetype_lock = PTHREAD_MUTEX_INITIALIZER;

extern int s_char_size;
...
int freetype_init()
{
    ...
    Set_Pixel_Sizes)
    {
        ...
        LOGD("do FT_New_Face");
        error = New_Face(pFTLib, FONTS_PATH, 2, &pFTFace);
...
{
    pthread_mutex_lock(&freetype_lock);
    ...
    if(Render_mode & Render_BOLD)
        Outline_Embolden( &pFTFace->glyph->outline, s_char_size*4);
...
void freetype_free(FontGraph *graph)
{
    if(graph->map)
        free(graph->map);
}
