...
int assisloopback_start(void)
{
	...
	gr_flip();
	pthread_create(&t1, NULL, assisloopback_thread, NULL);
...
int mainloopback_start(void)
{
	...
	gr_flip();
	pthread_create(&t1, NULL, mainloopback_thread, NULL);
	ret = ui_handle_button(TEXT_PASS, NULL,TEXT_FAIL);//, TEXT_GOBACK
	sem_post(&g_sem);
...
void* loopback_thread(int inrouth)
{
    ...
    do{
        ...
        property_get("persist.vendor.audioloop.test.result", value, "0");
        re = atoi(value);
        LOGD("persist.vendor.audioloop.test.result calue is: %s, atoi re is :%d!", value, re);
        if(1 == re)
            loop_result = RL_PASS;
        else
            loop_result = RL_FAIL;
    ...
    if(loop_result == RL_PASS){
        property_set("persist.vendor.audioloop.test.result", "0");
...
int mainloopback_start(void)
{
    ...
    row = ui_show_text(row, 0, TEXT_LB_MICSPEAKER);
    gr_flip();
    pthread_create(&t1, NULL, loopback_thread, (void*)0x80000004);
    pthread_join(t1, NULL); /* wait "handle key" thread exit. */
...
int test_assisloopback_start_common(void)
{
    /*char buff[64] = {0};
    int ret = 0;
    int row = 2;

    if(chnl_is_support(TEST_AUDIO_CALLBACK_DESC))
    {
    ...
    */
	int ret = 0;
	ret = assisloopback_start();
	return ret;

}

int test_mainloopback_start_common(void)
{
    /*char buff[64] = {0};
    int ret = 0;
    int row = 2;
