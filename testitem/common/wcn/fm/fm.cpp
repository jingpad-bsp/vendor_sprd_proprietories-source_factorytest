...
static int COM_set_mute(int fd, int mute)
 ...
 static int fm_show_play_stat(int freq, int inpw)
 {
	 ...
	 snprintf(text, sizeof(text), "%s:%d.%dMHz", TEXT_FM_FREQ, freq / 10, freq % 10);			 /*show channel*/
	 row = ui_show_text(row, 0, text);

	 memset(text, 0, sizeof(text));
	 snprintf(text, sizeof(text), "%s:%d(dBm)", TEXT_FM_STRE, inpw);		 /*show rssi*/
	 row = ui_show_text(row, 0, text);
	 ...
	 if ((inpw >= -105) && (inpw < 0)) {
		 ...
		 ui_set_color(CL_GREEN);
		 memset(text, 0, sizeof(text));
		 snprintf(text, sizeof(text), "%s>=-105(dBm)", TEXT_FM_STRE);		  /*show rssi*/
		 row = ui_show_text(row, 0, text);
 ...
 static int fm_headset_inserted(struct input_event *event)
 {
	 int ret = -1;
	 LOGD("type=%d; value=%d; code=%d\n",event->type, event->value, event->code);

	 if(event->type == EV_SW) {
		 if(SPRD_HEADSET_INSERT == event->code)
		 {
			 ret = event->value;
		 }
	 }
 ...
 static int fm_headset_check(void)
 {
	 int ret = -1, fd =-1;
	 int tmp = 0;
	 fd_set rfds;
   ...
   ui_show_button(NULL, NULL,TEXT_FAIL);
	 ...
	 if(strlen(getSensroTypeName(SENSOR_HDE))){
		 fd = get_sensor_name(getSensroTypeName(SENSOR_HDE));
		 LOGD("------------------");
		 if(fd < 0) {
	 ...
	 while(thread_run==1) {
		 ...
		 if(ret > 0 && FD_ISSET(fd, &rfds)) {
			 ...
			 if (ret == sizeof(event)) {
				 tmp = fm_headset_inserted(&event);
				 LOGD("headset event: tmp= %d",  tmp);
				 if(1 == tmp) {
					 ret = tmp;
					 break;
				 }
 ...
 void *fm_start(void *)
 {
	 ...
	 LOGD("mmitest freq=%d, fm marlin test",freq);
	 ret = fm_check_headset(HEADSET_OPEN, NULL);		 /*open headset device*/
	 if (ret < 0) {
		LOGD("mmitest open %s failed! %d IN", SPRD_HEADSET_SWITCH_DEV, __LINE__);
		ret = fm_headset_check();
		if(ret != 1){
			LOGD("mmitest headset check failed");
			ui_push_result(RL_FAIL);
			return NULL;
		}
	 } else {
		...
		while (0 == headset_in&&1 == thread_run) {
			LOGD("mmitest headset out, %d IN", __LINE__);
		...
		if (0 != ret) {
			LOGD("mmitest close %s failed! %d IN", SPRD_HEADSET_SWITCH_DEV, __LINE__);
			ui_push_result(RL_FAIL);
			return NULL;
		}
	 }
	 LOGD("mmitest headset in, %d IN", __LINE__);
	 ...
	 fm_show_searching(STATE_CLEAN);
	 ret=fm_show_play_stat(freq, rssi); 		  /*display rssi*/

	 return NULL;
 }

int test_fm_start_common(void)
{
     int ret = 0;
     pthread_t t1;
     row = 2;
     ...
     thread_run=1;
     setPlugState(1);
     pthread_create(&t1, NULL, fm_start, NULL);
     usleep(500*1000);
...
static void fm_show_play_stat(int freq, int inpw)
{
    ...
    ui_set_color(CL_GREEN);
    memset(text, 0, sizeof(text));
    snprintf(text, sizeof(text), "%s:%d.%dMHz", TEXT_FM_FREQ, freq / 100, freq % 100);           /*show channel*/
    row = ui_show_text(row, 0, text);
...
static int fm_headset_inserted(struct input_event *event)
{
	...
	if(event->type == EV_SW) {
		if((event->code == SPRD_HEADSET_INSERT)) {
			ret = event->value;
		}
	}
...
static int fm_headset_check(void)
{
	...
	if(strlen(HED_DEV_NAME)){
		fd = get_sensor_name(HED_DEV_NAME);
		LOGD("------------------");
		if(fd < 0) {
			ui_push_result(RL_FAIL);
			ui_set_color(CL_RED);
